...
def approval_pending(request):
    if not hasattr(request.user, 'vendor') or request.user.vendor.is_approved:
...
def dashboard(request):
    products = store_models.Product.objects.filter(vendor=request.user).order_by('-date')[:5]  # Show only recent products
    orders = store_models.Order.objects.filter(vendors=request.user, payment_status="Paid").order_by('-date')[:5]  # Show recent orders
    ...
    reviews = store_models.Review.objects.filter(product__vendor=request.user).order_by('-date')[:5]  # Show recent reviews
    ...
    # Add categories created by this vendor if you have that field
    # If not, you might want to add vendor field to Category model
    categories = store_models.Category.objects.all()[:5]  # Show recent categories or all categories
...
def profile(request):
    ...
    if request.method == "POST":
        ...
        if vendor_license != None and request.user.profile.user_type == "Vendor":
...
def categories(request):
    categories_list = Category.objects.filter(vendor=request.user)
...
def category_detail(request, id):
    ...
    products = category.products.all()  # Now using the property we defined
